...
class DecisionTreeC45:
        ...
        for feature in range(n_features):
            ...
            for value in feature_values:
                ...
                if len(left_indices) == 0 or len(right_indices) == 0:
                    continue
                info_gain = self._gain_ratio(y, y[left_indices], y[right_indices])
    ...
    def _information_gain(self, y, y_left, y_right):#信息增益
        p = len(y_left) / len(y)
        return self._entropy(y) - p * self._entropy(y_left) - (1 - p) * self._entropy(y_right)
    
    def _gain_ratio(self, y, y_left, y_right):#信息增益比
    ...
    def _split_info(self, y_left, y_right):
        ...
        return - (p_left * log2(p_left) + p_right * log2(p_right)) if p_left > 0 and p_right > 0 else 0
...
dt = DecisionTreeC45(max_depth=3)
